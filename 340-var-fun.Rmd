...
En este capítulo se mostrará como usar el paquete `nlme` de @R-nlme para modelar heterocedasticidad.
...
En esta sección vamos a simular datos de un modelo lineal mixto en el cual $\sigma^2_y$ dependa de una variable para luego estimar los parámetros del modelo.

## Ejemplo: $\sigma^2_y$ dependiendo de una variable cuantitativa  {-}
En este ejemplo vamos a simular observaciones $n_i=10$ observaciones para $G=20$ grupos (en total 200 obs) que tengan la estructura mostrada abajo. El objetivo del ejemplo es modelar la varianza $\sigma^2_y$.
...
```{r}
plot(fit0, resid(., type = "p") ~ x, abline = 0)
```
...
```{r}
plot(fit1, resid(., type = "p") ~ x, abline = 0)
```
