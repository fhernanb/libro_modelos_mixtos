...
Ahora vamos a simular 20 conjuntos de datos bajo $H_0$ verdadera y luego calcularemos los `lrt` para así tener la distribución real de ellos bajo la hipótesis nula verdadera.

```{r}
k <- 20
lrts <- numeric(k)
for (i in 1:k) {
  pseudo_datos <- gen_dat_b0(n=10, m=10, 
                             beta0=4.64931, beta1=-6.00175, 
                             sigma=2.50842, sigmab0=0)
  m1 <- gls(y ~ x, data=pseudo_datos)
