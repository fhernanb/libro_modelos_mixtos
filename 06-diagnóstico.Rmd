...
Por lo general, es suficiente con interpretar de forma visual una gráfica de residuos contra valores estimados para comprobar la validez del supuesto de varianza constante de los residuos. Sin embargo, existen pruebas que pueden proporcionar una justificación adicional al análisis. Dichas pruebas plantean el siguiente constraste de hipótesis: 

$$H_{0}: \sigma^{2}_{e_{1}} = \sigma^{2}_{e_{2}}$$

$$H_{A}: \sigma^{2}_{e_{1}} \ne \sigma^{2}_{e_{2}}$$

Así, $H_{0}$ (la hipótesis nula) sugiere una varianza constante de los residuos. A continuación se describen algunas de estas pruebas:

```{r Pruebas de homogeneidad de varianzas, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE}

Tabla_1 <- tibble::tribble(
  ~x, ~y,
  'Contraste de razón de varianzas', 'var.test( )',
  'Prueba de Levene', 'leveneTest( )',
  'Prueba de Bartlett', 'bartlett.test( )',
  'Prueba de Brown-Forsyth', 'hov( )',
  'Prueba de Fligner-Killeen', 'fligner.test( )'
)

kable(Tabla_1, col.names = c('Prueba', 'Función en R')) %>%
  kable_styling(full_width = FALSE) %>%
