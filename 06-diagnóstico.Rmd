...
### Gráfica básica de diagnóstico

La gráfica de residuos contra los valores estimados se considera la gráfica de diagnóstico más básica. A continuación podrá observar un ejemplo de la misma:
...
Imagen_1 <- readJPEG('images/Graf_correcta.jpg', native = TRUE)
...
Imagen_2 <- readJPEG('images/Graf_incorrecta_1.jpg', native = TRUE)
...
Imagen_3 <- readJPEG('images/Graf_incorrecta_2.jpg', native = TRUE)
...
Imagen_4 <- readJPEG('images/Graf_incorrecta_3.jpg', native = TRUE)
Imagen_4 <- rasterGrob(Imagen_4, interpolate=FALSE)

Graf_imagen_4 <- data.frame(
  x = c(1:2), y = c(1:2)
) %>%
  ggplot(aes(x, y)) +
  geom_blank() +
  ...
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank()) +
  annotation_custom(Imagen_4, xmin = 1.0, xmax = 2.0, ymin = 1.0, ymax = 2.0)

grid.arrange(Graf_imagen_1, Graf_imagen_2, Graf_imagen_3, Graf_imagen_4, ncol = 2)
```

Si se cumplen los supuestos, los residuos se dispersarán de forma aleatoria alrededor de la línea central como si se tratara de una nube de puntos, sin un patrón obvio, al igual a lo observado anteriormente en la gráfica superior izquierda.

Si hay un patrón no aleatorio, su naturaleza puede identificar posibles problemas con el modelo. Por ejemplo si hay una curvatura presente (al igual a lo observado en la gráfica superior derecha), es posible que la relación entre la variable respuesta y el regresor no sea de forma lineal. Por lo tanto un modelo lineal, si este fuera el caso, no describiria adecuadamente la relación entre dichas variables.

Luego si los residuos cambian de forma abrupta a medida que aumenta los valores estimados (como la gráfica inferior izquierda), este sería un claro de ejemplo de falta de homogeneidad de la varianza.

Por último, un patrón inusual también puede ser causado por un valor atípico. En el ejemplo visual anterior (gráfica inferior derecha), se muestra un valor atípico obvio.

En el ejemplo planteado usando la base de datos `hsb`, esta gráfica se muestra en la parte superior e inferior izquierda de la siguiente figura, basados en dos tipos de residuos (ordinarios y pearson):
...
grid.arrange(Grafica_A, Grafica_B, Grafica_C, Grafica_D, ncol = 2)
```
...
En general, los gráficos anteriores se evalúan de forma informal con respecto a la presencia o ausencia de patrones específicos y/o puntos de datos periféricos o aislados. Respecto a la línea de referencia en cero y la línea ajustada mediante el método de `loess`, estas deberían parecerse. Por otro lado en cuanto al gráfico de caja y bigotes (parte superior e inferior derecha de la anterior figura), estas deberian tener aproximadamente el mismo centro y distancia intercuartil.

```{block2, type='rmdnote'}

La gráfica de residuos anterior es útil para revelar problemas, pero menos útil para determinar la naturaleza exacta del problema. En consecuencia, se presentaran otras gráficas de diagnóstico para sugerir mejoras a un modelo.
```

### Gráfica... 

Una variación de la gráfica residual básica es la gráfica...

```{r Imagen distribución normal, fig.align = 'center', eval = TRUE, echo = FALSE, message = FALSE}

library(knitr)
include_graphics('images/Graf_correcta_2.gif')
```
