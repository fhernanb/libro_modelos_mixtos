...
Los análisis de normalidad, también denominados como contraste de normalidad, son otro tipo de supuesto del modelo de regresión. Al igual que el supuesto de varianza constante de los residuos, en el análisis de normalidad se suele emplear representaciones gráficas. A continuación podrá observar un ejemplo de dichas representaciones en el caso del cumplimiento o no del supuesto de normalidad:
...
En el ejemplo planteado usando la base de datos `hsb`, esta gráfica se muestra en la figura a continuación.
...
Grafica_A <- ggplot(data = hsb, aes(sample = res_pearson)) +
  stat_qq(distribution = qnorm, color = "yellow", alpha = 0.4, size = 4) +
  stat_qq_line(distribution = qnorm, color = "black", size = 1.2) +
  labs(x = "Cuantiles teóricos de una distribución normal", y = "Cuantiles de la distribución observada") +
  theme_bw() +
