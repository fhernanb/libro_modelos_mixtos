...
Si se cumplen los supuestos, los residuos se dispersarán de forma aleatoria alrededor de la línea central como si se tratara de una nube de puntos, sin un patrón obvio (figura 6.7 __A__). Si hay un patrón no aleatorio, su naturaleza puede identificar posibles problemas con el modelo. Por ejemplo si hay una curvatura presente (figura 6.7 __B__), es posible que la relación entre la variable respuesta y el regresor no sea de forma lineal. O bien, si los residuos cambian de forma abrupta a medida que aumenta los valores estimados, este sería un claro de ejemplo de falta de homogeneidad de la varianza. Finalmente, un patrón inusual también puede ser causado por un valor atípico. En el ejemplo visual anterior (figura 6.7 __C__), se muestra un valor atípico obvio.
...
En el ejemplo planteado usando la base de datos `hsb`, esta gráfica se muestra en la figura 6.9 <!--__B__ a continuación:
